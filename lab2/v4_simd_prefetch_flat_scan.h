...
inline float dot_product_simd_prefetch(const float* a, const float* b, size_t vecdim) {
#if defined(__aarch64__) || defined(__ARM_NEON)
...
flat_search_simd_prefetch(float* base, float* query, size_t base_number, size_t vecdim, size_t k) {
    std::priority_queue<std::pair<float, uint32_t>> q;

    constexpr size_t PREFETCH_DISTANCE = 16;

    for (size_t i = 0; i < base_number; ++i) {
        size_t pf_i = i + PREFETCH_DISTANCE;
        if (pf_i < base_number) {
            __builtin_prefetch(base + pf_i * vecdim, 0, 1);
        }

        const float* base_vec = base + i * vecdim;

        float dis = 1.0f - dot_product_simd_prefetch(base_vec, query, vecdim);
