...
inline float dot_product_simd_unroll_multiacc(const float* a, const float* b, size_t vecdim) {
#if defined(__aarch64__) || defined(__ARM_NEON)
    ...
    // 每轮处理 16 个 float：4 个 NEON 向量 + 4 个累加器
...
flat_search_simd_unroll_multiacc(float* base, float* query, size_t base_number, size_t vecdim, size_t k) {
    ...
    for (size_t i = 0; i < base_number; ++i) {
        ...
        float dis = 1.0f - dot_product_simd_unroll_multiacc(base_vec, query, vecdim);
