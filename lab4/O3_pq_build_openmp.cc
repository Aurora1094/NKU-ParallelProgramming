...
SearchResult search_one_query_ivf_simd(
    int i,
    float* base,
    ...
    auto res = ivf_search_simd_rerank(
        base,
        ivf_index,
        test_query + static_cast<size_t>(i) * vecdim,
        base_number,
...
void* pthread_worker_ivf_simd(void* arg) {
    ThreadArgIVF* a = static_cast<ThreadArgIVF*>(arg);

    int cnt = 0;

    for (int i = a->tid; i < a->test_number; i += a->num_threads) {
        (*a->results)[i] = search_one_query_ivf_simd(
            i,
            a->base,
            *(a->ivf_index),
            a->test_query,
...
SearchResult search_one_query_ivf_pq_simd(
    int i,
    float* base,
    const IVFIndex& ivf_index,
    const PQIndex& pq_index,
    ...
    float* query = test_query + static_cast<size_t>(i) * vecdim;

    auto res = ivf_pq_search_simd_rerank(
        base,
        ivf_index,
        pq_index,
        query,
    ...
    for (size_t j = 0; j < test_gt_d; ++j) {
        gt_set.insert(static_cast<uint32_t>(
            test_gt[static_cast<size_t>(i) * test_gt_d + j]
        ));
    }
    ...
    SearchResult result;
    result.recall = static_cast<float>(correct) / static_cast<float>(k);
    result.latency = latency;

    return result;
}

struct ThreadArgIVFPQ {
    int tid;
    int num_threads;
    int test_number;
...
void* pthread_worker_ivf_pq_simd(void* arg) {
    ThreadArgIVFPQ* a = static_cast<ThreadArgIVFPQ*>(arg);

    int cnt = 0;

    for (int i = a->tid; i < a->test_number; i += a->num_threads) {
        (*(a->results))[i] = search_one_query_ivf_pq_simd(
            i,
            a->base,
            *(a->ivf_index),
            *(a->pq_index),
...
int main(int argc, char *argv[])
{
    ...
    std::cout << "openmp num_threads:" << num_threads << "\n";
    ...
    // const size_t ivf_train_samples = 10000;
    // const int ivf_kmeans_iters = 3;
    // const size_t ivf_nprobe = 16;


    // ===============================
    // V8: IVF-PQ-SIMD 离线索引构建
    // ===============================
    const size_t ivf_nlist = 256;
    const size_t ivf_train_samples = 10000;
    const int ivf_kmeans_iters = 3;
    ...
    const int pq_kmeans_iters = 3;
    const size_t rerank_p = 2000;
    const int pq_build_threads = 8;

    // auto ivf_build_start = std::chrono::high_resolution_clock::now();

    // IVFIndex ivf_index = build_ivf_index_basic(
    //     base,
    //     base_number,
    //     vecdim,
    //     ivf_nlist,
    ...
    // auto ivf_build_end = std::chrono::high_resolution_clock::now();

    // double ivf_build_us =
    //     std::chrono::duration<double, std::micro>(
    //         ivf_build_end - ivf_build_start
    //     ).count();
// 记得删
    // std::cout << "method: IVF-SIMD + Pthread query-level parallelism\n";
    // std::cout << "method: IVF-PQ-SIMD + Pthread query-level parallelism\n";
    // std::cout << "method: IVF-PQ-SIMD + OpenMP query-level parallelism\n";
    // std::cout << "method: IVF-SIMD + OpenMP query-level parallelism, schedule(static)\n";
    // std::cout << "method: IVF-SIMD + OpenMP query-level parallelism, schedule(dynamic,8)\n";
    // std::cout << "ivf_nlist: " << ivf_nlist
    //           << ", ivf_nprobe: " << ivf_nprobe
    //           << ", ivf_train_samples: " << ivf_train_samples
    //           << ", ivf_kmeans_iters: " << ivf_kmeans_iters << "\n";
    // std::cout << "ivf index build time (us): " << ivf_build_us << "\n";
    // std::cout << "method: Flat-SIMD + OpenMP query-level parallelism, schedule(static)\n";
    // std::cout << "method: PQ-SIMD + OpenMP query-level parallelism, schedule(static)\n";
    std::cout << "method: PQ-SIMD + OpenMP query-level + OpenMP PQ-build\n";
    // print_ivf_list_stats(ivf_index);
...
PQIndex pq_index = build_pq_index_openmp_v6_2(
    base,
    base_number,
    vecdim,
    pq_M,
...
std::cout << "pq_M: " << pq_M
          ...
          << ", pq_kmeans_iters: " << pq_kmeans_iters
          << ", rerank_p: " << rerank_p << "\n";
...
// v7 IVF-SIMD + Pthread query-level parallelism
// std::vector<pthread_t> threads(num_threads);
// std::vector<ThreadArgIVF> args(num_threads);
...
//     args[t].test_gt_d = test_gt_d;
//     args[t].k = k;
//     args[t].nprobe = ivf_nprobe;

//     args[t].results = &results;
//     args[t].processed_count = 0;

//     pthread_create(&threads[t], nullptr, pthread_worker_ivf_simd, &args[t]);
// }

// v8 IVF-PQ-SIMD + Pthread query-level parallelism
// std::vector<pthread_t> threads(num_threads);
// std::vector<ThreadArgIVFPQ> args(num_threads);
...
// v8 IVF-PQ-SIMD + OpenMP query-level parallelism
omp_set_num_threads(num_threads);

// std::vector<int> omp_processed_count(num_threads, 0);

// #pragma omp parallel for schedule(static)
// for (int i = 0; i < static_cast<int>(test_number); ++i) {
//     int tid = omp_get_thread_num();

//     results[i] = search_one_query_ivf_pq_simd(
//         i,
//         base,
//         ivf_index,
//         pq_index,
...
//     omp_processed_count[tid]++;
// }

// IVF-SIMD + OpenMP query-level parallelism
omp_set_num_threads(num_threads);

std::vector<int> omp_processed_count(num_threads, 0);

// v7 IVF-SIMD + OpenMP query-level parallelism
// #pragma omp parallel for schedule(static)
// #pragma omp parallel for schedule(dynamic, 8)
// for (int i = 0; i < static_cast<int>(test_number); ++i) {
//     int tid = omp_get_thread_num();

//     results[i] = search_one_query_ivf_simd(
//         i,
//         base,
//         ivf_index,
//         test_query,
...
// v4 Flat-SIMD + OpenMP query-level parallelism
#pragma omp parallel for schedule(static)
for (int i = 0; i < static_cast<int>(test_number); ++i) {
    int tid = omp_get_thread_num();
    
    // v4
    // results[i] = search_one_query_flat_simd(
    //     i,
    //     base,
    //     test_query,
    //     test_gt,
    ...
    // v6_2
    results[i] = search_one_query_pq_simd(
        i,
        base,
        pq_index,
        test_query,
...
for (int t = 0; t < num_threads; ++t) {
    std::cout << "omp thread " << t
              << " processed queries: "
              << omp_processed_count[t] << "\n";
}
