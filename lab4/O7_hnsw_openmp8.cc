...
SearchResult search_one_query_hnsw(
    int i,
    ...
    while (!res.empty()) {
        uint32_t id = static_cast<uint32_t>(res.top().second);
        res.pop();
...
void* pthread_worker_hnsw(void* arg) {
    ThreadArgHNSW* a = static_cast<ThreadArgHNSW*>(arg);

    int cnt = 0;

    for (int i = a->tid; i < a->test_number; i += a->num_threads) {
        (*(a->results))[i] = search_one_query_hnsw(
            i,
            a->hnsw_index,
            a->test_query,
            a->test_gt,
...
int main(int argc, char *argv[])
{
    ...
    const int num_threads_for_search = 8;
    ...
    // ===============================
    // HNSW baseline + Pthread query-level parallelism
    // ===============================
    const int hnsw_M = 16;
    const int hnsw_efConstruction = 150;
    const int hnsw_efSearch = 100;
    ...
    for (size_t i = 0; i < base_number; ++i) {
        hnsw_index->addPoint(
            base + static_cast<size_t>(i) * vecdim,
            i
        );
    }

    hnsw_index->setEf(hnsw_efSearch);

    auto hnsw_build_end = std::chrono::high_resolution_clock::now();

    double hnsw_build_us =
        std::chrono::duration<double, std::micro>(
            hnsw_build_end - hnsw_build_start
        ).count();

// 记得删
// std::cout << "method: HNSW + Pthread query-level parallelism\n";
std::cout << "method: HNSW + OpenMP query-level parallelism, schedule(static)\n";
std::cout << "hnsw_M: " << hnsw_M
          << ", hnsw_efConstruction: " << hnsw_efConstruction
          << ", hnsw_efSearch: " << hnsw_efSearch << "\n";
std::cout << "hnsw index build time (us): " << hnsw_build_us << "\n";
...
// HNSW + Pthread query-level parallelism
// std::vector<pthread_t> threads(num_threads);
// std::vector<ThreadArgHNSW> args(num_threads);
...
for (int i = 0; i < static_cast<int>(test_number); ++i) {
    ...
    results[i] = search_one_query_hnsw(
        i,
        hnsw_index,
        test_query,
        test_gt,
