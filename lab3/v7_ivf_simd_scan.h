...
ivf_pq_search_simd_rerank(
    ...
    if (nprobe > ivf_index.nlist) {
        nprobe = ivf_index.nlist;
    }
    ...
    // ======================================================
    // 1. IVF coarse：计算 query 到所有 IVF centroid 的相似度，
    //    选出 top-nprobe 个 inverted list
    // ======================================================
    std::priority_queue<
        ScorePair,
        std::vector<ScorePair>,
    ...
    for (size_t c = 0; c < ivf_index.nlist; ++c) {
        const float* centroid = ivf_index.centroids.data() + c * vecdim;
    ...
    // ======================================================
    // 2. PQ LUT：对当前 query 构建 PQ 查表表项
    // ======================================================
    std::vector<float> lut;
    build_query_lut_v6_2(pq_index, query, lut);

    // ======================================================
    // 3. 在 IVF 选出的倒排链表内做 PQ-ADC 粗排
    //    只扫描 selected inverted lists，不再扫描全量 base
    // ======================================================
    std::priority_queue<
        ScorePair,
        std::vector<ScorePair>,
    ...
    for (uint32_t cid : probe_lists) {
        const std::vector<uint32_t>& ids = ivf_index.inverted_lists[cid];
        ...
        for (uint32_t id : ids) {
            ...
            const uint8_t* code_i =
                pq_index.codes.data() + static_cast<size_t>(id) * pq_index.M;

            for (size_t m = 0; m < pq_index.M; ++m) {
                uint8_t code = code_i[m];
                score += lut[m * pq_index.Ks + static_cast<size_t>(code)];
            }

            if (coarse_q.size() < rerank_p) {
                coarse_q.push({score, id});
    ...
    // ======================================================
    // 4. 对 PQ 粗排得到的 top-rerank_p 候选做 float SIMD 精排
    // ======================================================
    std::priority_queue<std::pair<float, uint32_t>> final_q;
